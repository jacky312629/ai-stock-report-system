...
def calculate_stock_score(cat, stock, sox_change, nasdaq_change, tsm_change, vix_change, dbc_change, crb_change, news_category_scores):
    score = 0
    tags = stock.get("tags", [])
    ...
    # 5. 新聞類股加權
    score += news_category_scores.get(cat, 0)
...
def pick_recommendation_categories(results, news_text, news_category_scores):
    selected = []

    sox = results.get("費城半導體", {}).get("change", 0) or 0
    nasdaq = results.get("NASDAQ", {}).get("change", 0) or 0
    vix = results.get("VIX", {}).get("change", 0) or 0
    dbc = results.get("原物料ETF", {}).get("change", 0) or 0
    crb = results.get("CRB指數", {}).get("change", 0) or 0

    # 1. 新聞類股分數優先
    if news_category_scores:
        positive_news_categories = [
            cat for cat, score in sorted(
                news_category_scores.items(),
                key=lambda x: x[1],
                reverse=True
            )
            if score > 0
        ...
        selected.extend(positive_news_categories[:3])

    # 2. 科技主線
    if sox > 0 or nasdaq > 0:
        selected.extend(["ai", "ip", "server", "cooling", "cpo"])

    # 3. 原物料主線
    if dbc > 0.5 or crb > 0.5:
        selected.extend(["steel", "plastic"])
    ...
    final_selected = []
    for cat in selected:
        if cat not in final_selected:
            final_selected.append(cat)
...
def pick_top_stocks(results, stock_candidates, selected_categories, news_category_scores):
    ...
    # 精選門檻
    MIN_FEATURED_SCORE = 10
    MIN_WATCH_SCORE = 7

    # 精選 3 檔：低於門檻不硬推
    featured_stocks = [
        s for s in sorted_stocks
        if s["score"] >= MIN_FEATURED_SCORE
    ][:3]
    ...
    # 推薦觀察最多 5 檔
    featured_names = {s["name"] for s in featured_stocks}

    watch_stocks = [
        s for s in sorted_stocks
        if s["name"] not in featured_names
        and s["score"] >= MIN_WATCH_SCORE
    ][:5]
...
def load_config(filepath="config.json"):
    ...
    try:
        ...
        logging.warning(f"找不到 {filepath}，使用預設設定")
        return {}
    except json.JSONDecodeError as e:
        logging.error(f"{filepath} JSON 格式錯誤: {e}")
        return {}
...
def classify_news_impact(title, rules):

    title_lower = title.lower()
    impact_rules = rules.get("impact_rules", [])
    ...
    for rule in impact_rules:
        ...
        if matched:
            return rule.get("impact", "中性")

    return "中性"


def analyze_news_to_category_scores(news_titles, news_rules):

    category_scores = {}

    impact_rules = news_rules.get("impact_rules", [])
    ...
    for news in news_titles:
        ...
        for rule in impact_rules:
            ...
            # 加分類股
            for cat in rule.get("boost_categories", []):
                category_scores[cat] = (
            ...
            # 扣分類股
            for cat in rule.get("penalty_categories", []):
                category_scores[cat] = (
                    category_scores.get(cat, 0)
                    - score
                )
...
def build_news_score_text(news_category_scores):
    if not news_category_scores:
        return "今日新聞未明顯影響類股權重"
    ...
    for cat, score in sorted(
        ...
        lines.append(f"{cat}：{sign}{score}")

    return "\n".join(lines)


def analyze_mega_cap_scores(config):
    mega_cap_map = config.get("mega_cap_map", {})
    category_scores = {}
    mega_cap_results = {}

    for symbol, rule in mega_cap_map.items():
        result = fetch_market_data([symbol])
        mega_cap_results[symbol] = result
        ...
        strong_threshold = rule.get("strong_threshold", 1.0)
        weak_threshold = rule.get("weak_threshold", -1.0)
    ...
    return category_scores, mega_cap_results


def merge_category_scores(*score_dicts):
    merged = {}

    for score_dict in score_dicts:
        for cat, score in score_dict.items():
            merged[cat] = merged.get(cat, 0) + score
...
def main():
    ...
    config = load_config()
    news_rules = load_config("news_rules.json")

    symbols_map = config.get("symbols_map", {})
    ...
    url = news_config.get("url", "https://www.cnbc.com/world/?region=world")
    max_news_count = news_config.get("max_count", 5)
    ...
    news_category_scores = analyze_news_to_category_scores(
        news_titles,
        news_rules
    )

    mega_cap_category_scores, _ = analyze_mega_cap_scores(config)

    combined_category_scores = merge_category_scores(
        news_category_scores,
        mega_cap_category_scores
    )

    print("\n=== Category Scores Debug ===")
    print(build_news_score_text(combined_category_scores))

    selected_categories = pick_recommendation_categories(
        results,
        news_text,
...
# ＝＝＝＝＝＝5.報告組裝＝＝＝＝＝＝
    ...
    if "偏空" in strategy or "保守" in strategy:
        top3_strategy_block = (
            "【今日操作策略】\n"
            "👉 市場風險偏高，暫不主動布局\n\n"
            "【精選3檔（觀察用）】\n"
        ...
        top3_strategy_block = build_top3_strategy_block(
            strategy,
            sorted_stocks
        )
    ...
    report_console += f"\n【美股新聞】\n{line_sub}\n"
    if news_titles:
        for i, news in enumerate(news_titles[:3], start=1):
            report_console += (
                f"{i}. "
                f"[{news['impact']}] "
                f"{news['title']}\n"
...
# =========================
    ...
    try:
        ...
        to_email = email_config.get("to", "jacky312619@gmail.com")
        subject = email_config.get("subject", "📊 每日美股盤前策略報告")
